...
Одной из последний версий YOLO является [YOLOv3](https://pjreddie.com/media/files/papers/YOLOv3.pdf). В ней авторы отказались от *fully-connected* слоя в конце, таким образом остались только свёрточные слои. Было добавлено 2 дополнительных выходных слоя для улучшения распознавания объектов разного размера.
...
![](https://habrastorage.org/webt/1_/zr/1r/1_zr1rmvg0nehl2-iahcbjn2aey.png)
 ...
 В ней также используется слой пулинга вместо свёрточного слоя с страйдом 2. Это делает её быстрее и компактнее(~ 35 Мб), а значит более предпочтительнее для моей задачи. 
